...
int orig_main (int argc, char *argv[], int absolute_par_no, blk64_t first_data_blk,
	blk64_t last_data_blk, blk64_t next_sb_blk)
{
	...
	fs->device_mask = absolute_par_no; // record in dir
...
int main (int argc, char *argv[]) {
	...
	multi_dev_name = strdup(argv[argc-1]);

	while((dev_name = strsep(&multi_dev_name, ",")) != NULL) {
		argv[argc-1] = dev_name;
		// get dev_size
  		ext2fs_get_device_size2(dev_name, 4096, &dev_size);

		partition_size = dev_size / n_partition;
	
		for(relative_par_no=n_partition-1; relative_par_no>=0; relative_par_no--) {
			first_data_blk = partition_size * relative_par_no;
			last_data_blk = first_data_blk + partition_size - 1;
			next_sb_blk = (relative_par_no == n_partition - 1) ? 0 : last_data_blk + 1;
			printf("main: relative_par_no-%d, first_data_blk-%llu, last_data_blk-%llu, next_sb_blk-%llu\n", relative_par_no, first_data_blk, last_data_blk, next_sb_blk);
			orig_main(argc, argv, par_sum+relative_par_no, first_data_blk, last_data_blk, next_sb_blk);
			printf("------------------------ %s partition %d done ------------------------\n\n", device_name, relative_par_no);
		}
		par_sum+=n_partition;
	}
